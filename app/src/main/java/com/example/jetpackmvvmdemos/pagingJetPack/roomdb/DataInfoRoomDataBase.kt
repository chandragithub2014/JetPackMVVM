...
import com.example.jetpackmvvmdemos.pagingJetPack.ioThread
   ...
   private class SRoomDatabaseCallback : RoomDatabase.Callback() {
    ...
    class PopulateDbAsyncTask(db: DataInfoRoomDataBase?) : AsyncTask<Unit, Unit, Unit>() {
        ...
        override fun doInBackground(vararg p0: Unit?) {
           ...
           // Log.d("DataInfoRoomDb",""+CHEESE_DATA.map{DataInfo(uid = 0, body="Info" , title = it)}.toMutableList())
           val list = CHEESE_DATA.map{DataInfo(uid = 0, body="Info" , title = it)}.toMutableList()
            Log.d("DataInfoRoomDB","ListSize::::"+list.size)
            for(index in 0..list.size-1){
                Log.d("DataInfoRoomDb","DataInfoRoom:::"+list[index])
            }
            noteDao?.insertAll(
                CHEESE_DATA.map{DataInfo(uid = 0, body="Info" , title = it)}.toMutableList())
...
private val CHEESE_DATA = arrayListOf(
    ...
    "Bresse Bleu", "Brick", "Brie", "Brie de Meaux", "Brie de Melun", "Brillat-Savari"
     ,"TestData","Test Data2"
    /*,
